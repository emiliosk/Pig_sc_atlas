...
def plot_markers(adata, tissue, prep_method, results_dir):
    ...
    #    sc.pl.rank_genes_groups_violin(adata, n_genes=20, jitter=False, show=False)
    #    pdf.savefig(bbox_inches='tight')
...
for tissue, ls_acc in single_nuclei.items():
        ...
        adata = integrate_files_mgi(tissue = tissue, 
                            ...
                            directory =  "/proj/rnaatlas/nobackup/private/EmilioTemp/pig_sc_109/single_nuclei_cr/", 
                            plot = True, use_filtered = True)
        adata =  integrated_qc(adata, tissue = tissue, results_path = results_path)
        adata.write_h5ad(results_path  + '/'+ tissue  + '_integrated.h5ad')

        
        
tissue = 'Lung'
results_path = '../results_qc/single_nuclei_MGI/'+ tissue
adata = sc.read_h5ad(tissue  + '_integrated.h5ad')
...
#check umap, to clusters, remove then the low quality unspecific clusters
sc.pl.umap(adata, color = ['leiden', 'total_counts'])

prep_method = 'snRNA-seq'
with open('../../../Notebooks/cell_markers_dict_dk.pkl', 'rb') as f:
        cell_markers_dict_dk = pickle.load(f)
sample = tissue.title() + '_' + prep_method
cell_markers_dict_dk = cell_markers_dict_dk[sample]
cell_markers_dict_dk = get_markers_in_adata(adata, cell_markers_dict_dk)
sc.pl.dotplot(adata, cell_markers_dict_dk, 'leiden',standard_scale = 'var', dendrogram = True)
...
#make sub of adata, excluding the low Quality clusters 0 and 1 in this case
adata_sub = adata[~adata.obs.leiden.isin(['0', '1']),:]

adata_sub.uns['log1p']['base'] = None
sc.pp.highly_variable_genes(adata_sub, min_mean=0.0125, max_mean=3, min_disp=0.5,  batch_key= 'sample') 
adata_sub.raw = adata_sub.raw
sc.tl.pca(adata_sub, svd_solver='arpack')

sc.external.pp.harmony_integrate(adata_sub, 'sample')

sc.pp.neighbors(adata_sub, use_rep='X_pca_harmony')

sc.tl.umap(adata_sub, random_state=4)
sc.tl.leiden(adata_sub)

adata_sub.write_h5ad('adata_sub_clustered.h5ad')
