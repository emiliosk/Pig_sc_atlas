...
def individual_qc(adata, acc, results_path = './',  plot = True):
    ...
    scrub = scr.Scrublet(adata.X, expected_doublet_rate=0.1, random_state=4)
...
def integrated_qc(adata, tissue, results_path, plot = True):
    ...
    if plot == True:
        ...
        pdf=PdfPages(results_path+"/" + tissue + "_figures_integrated_qc.pdf")
...
def preprocessing(adata, tissue,  results_path, filter_genes = True, filter_cells = True, plot = True):
    ...
    if plot == True:
        pdf=PdfPages(results_path+ "/"  + tissue + "_figures_pre_process.pdf")
    ...
    sc.pp.regress_out(adata, [ 'pct_counts_mt']) #Regress out effects of total counts per cell and the percentage of mitochondrial genes expressed
...
for tissue in tissues:
    try:
        os.mkdir('../results_qc/single_cell/'+ tissue)
        results_path = '../results_qc/single_cell/'+ tissue
        adata = load_file_10x(acc = tissue, directory = "/proj/rnaatlas/nobackup/private/EmilioTemp/pig_sc_109/single_cell/", use_filtered = True,  min_cells = 0, min_genes = 0)
        adata = individual_qc(adata, acc = tissue, results_path = results_path,  plot = True)
        adata =  integrated_qc(adata, tissue = tissue, results_path = results_path)
        adata = preprocessing(adata, tissue = tissue, results_path = results_path, filter_genes = True, filter_cells = True, plot = True)
        adata
        adata.write_h5ad(results_path  + '/'+ tissue  + '_preprocessed.h5ad')
        adata = plot_markers(adata, tissue = tissue, prep_method = "scRNA-seq", results_dir = results_path )
    except:
        pass
