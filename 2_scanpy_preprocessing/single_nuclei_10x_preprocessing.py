...
sc.set_figure_params(scanpy=True, dpi=60)
...
def individual_qc(adata, acc, results_path = './',  plot = True):
    ...
    scrub = scr.Scrublet(adata.X, expected_doublet_rate=0.075, random_state=4)
...
def preprocessing(adata, tissue,  res_path, filter_genes = True, filter_cells = True, plot = True):
    ...
    #sc.pp.scale(adata)
    #sc.pp.regress_out(adata, ['pct_counts_mt'])#'total_counts', 'pct_counts_mt'])
...
single_nuclei = {'Frontal lobe': ["frontal"],
...
for tissue, ls_acc in single_nuclei.items():
    os.mkdir('../results_qc/single_nuclei_10x_star/'+ tissue)
    results_path = '../results_qc/single_nuclei_10x_star/'+ tissue
    adata = integrate_files_mgi(tissue = tissue, 
                        ls_acc = ls_acc, 
                        min_cells = 0, min_genes = 0, 
                        results_path = results_path, 
                        directory =  "/proj/rnaatlas/nobackup/private/EmilioTemp/pig_sc_109/single_nuclei_10x_star/", 
